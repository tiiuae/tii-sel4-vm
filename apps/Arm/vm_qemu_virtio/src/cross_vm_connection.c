...
{
    struct camkes_crossvm_connection *connections = cookie;
    ...
    int err = register_async_event_handler(connections[0].consume_badge,
                                           consume_callback,
                                           &connections[0]);
...
DEFINE_MODULE(cross_vm_connections, connections, init_cross_vm_connections)
