...
#if defined(CONFIG_VM_SWIOTLB)
extern uintptr_t swiotlb_gpa;
extern size_t swiotlb_size;
...
static int generate_fdt_swiotlb(void *gen_fdt)
{
     ...
     */
    int err = fdt_generate_memory_node(gen_fdt, swiotlb_gpa, swiotlb_size);
    if (err) {
        return err;
    }
    ...
    uint32_t phandle;
    char name[64];
    sprintf(name, "swiotlb@%"PRIxPTR, swiotlb_gpa);
    err = fdt_generate_reserved_node(gen_fdt, name, "restricted-dma-pool",
                                     swiotlb_gpa, swiotlb_size, &phandle);
    if (err) {
        return err;
    }
